...
if [ $OSVersion == 11  ] || [ $OSVersion == 12 ] || [ $OSVersion == "20.04" ] || [ $OSVersion == "22.04" ]; then
...
# Ask the user whether the server should be used as a mail server with Mailcow.
if WhipYesNo "${lang_btn_yes}" "${lang_btn_no}" "${lang_configmailserver_title}" "${lang_configmailserver_messageyesno}"; then ConfigMailserverRole=true; fi
